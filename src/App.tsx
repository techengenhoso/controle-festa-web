        ...
        <div className="layout">
          ...
          <div className="content">
            {section === 'parties' && <Section title="Festas" subtitle="Gerencie eventos" actions={<><Button onClick={() => openPartyForm()}>Criar festa</Button><Button variant="secondary" onClick={() => setShowArchived((value) => !value)}>{showArchived ? 'Ocultar festas arquivadas' : 'Mostrar festas arquivadas'}</Button></>}>
              {visibleParties.length === 0 ? <div className="empty-state"><strong>Nenhuma festa criada ainda</strong><p>Crie a primeira festa para liberar comandas, cardápios, consumo e saldos.</p></div> : <div className="grid-list">{visibleParties.map((party) => <button className="card card-button" key={party.id} onClick={() => openPartyDetails(party.id)}><div><h3>{party.name}</h3><p>{formatStoredDate(party.date)}</p><div className="badges"><span className={party.active ? 'badge-active' : 'badge-inactive'}>{party.active ? 'Ativa' : 'Inativa'}</span>{party.archived && <span className="badge-archived">Arquivada</span>}</div></div></button>)}</div>}
            </Section>}
