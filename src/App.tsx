        ...
        <div className="layout">
          ...
          <div className="content">
            {section === 'parties' && <Section title="Festas" actions={<div className="party-actions"><Button onClick={() => openPartyForm()}>Criar festa</Button><Button variant="secondary" onClick={() => setShowArchived((value) => !value)}>Arquivo</Button></div>}>
              {visibleParties.length === 0 ? <div className="empty-state"><strong>Nenhuma festa criada ainda</strong><p>Crie a primeira festa para liberar comandas, cardápios, consumo e saldos</p></div> : <div className="grid-list">{visibleParties.map((party) => <button className="card card-button" key={party.id} onClick={() => openPartyDetails(party.id)}><div className="party-card-content"><h3>{party.name}</h3><div className="party-card-meta"><p>{formatStoredDate(party.date)}</p><div className="badges">{!party.archived && <span className={party.active ? 'badge-active' : 'badge-inactive'}>{party.active ? 'Ativa' : 'Inativa'}</span>}{party.archived && <span className="badge-archived">Arquivada</span>}</div></div></div></button>)}</div>}
            </Section>}
