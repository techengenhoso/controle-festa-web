...
export function normalizeParty(value: Partial<Party>): Party {
  const date =
    ...
    "";
  const tabs = sortTabs(
    Array.isArray(value.tabs) ? value.tabs.map(normalizeTab) : [],
  );
  const menu = sortMenu(
    Array.isArray(value.menu) ? value.menu.map(normalizeMenuItem) : [],
  );
  ...
  const consumptions = Array.isArray(value.consumptions)
    ? value.consumptions
        ...
        .filter(
          (consumption) =>
            tabIds.has(consumption.tabId) &&
            menuById.has(consumption.menuItemId),
        )
        .map((consumption) => {
...
export function normalizeAppData(value: Partial<AppData> | Party[]): AppData {
  ...
  const selectedPartyId = Array.isArray(value)
    ? undefined
    : value.selectedPartyId;
  const selectedActiveParty = normalizedParties.find(
    (party) => party.id === selectedPartyId && party.active && !party.archived,
  );
  const activeParty =
    selectedActiveParty ??
    normalizedParties.find((party) => party.active && !party.archived);
  const activePartyId = activeParty?.id;
  const partiesWithSingleActive = normalizedParties.map((party) => ({
    ...party,
    active: party.id === activePartyId,
  }));
  const selectedTabId = Array.isArray(value) ? undefined : value.selectedTabId;
  const selectedTabExists = activeParty?.tabs.some(
    (tab) => tab.id === selectedTabId && tab.active,
  );

  return {
    parties: partiesWithSingleActive,
    selectedPartyId: activePartyId,
    selectedTabId: selectedTabExists ? selectedTabId : undefined,
  };
}
