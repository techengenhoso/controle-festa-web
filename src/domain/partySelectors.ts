...
export function resolveSelectedTab(appData: AppData, activeTabs: Tab[]) {
  return activeTabs.some((tab) => tab.id === appData.selectedTabId) ? appData.selectedTabId ?? '' : activeTabs[0]?.id ?? ''
}

export function sumConsumptionsByTab(party: Party | undefined, tabId: string) {
  return party?.consumptions.filter((item) => item.tabId === tabId).reduce((sum, item) => sum + item.price, 0) ?? 0
}
...
export function getBalanceTotals(party: Party | undefined, tabs: Tab[]) {
  return {
    consumed: party?.consumptions.filter((item) => tabs.some((tab) => tab.id === item.tabId)).reduce((sum, item) => sum + item.price, 0) ?? 0,
    minimum: tabs.reduce((sum, tab) => sum + getRemainingMinimum(party, tab), 0),
  }
}

export function getPartyHeader(party: Party | undefined, options: { showTabBalance: boolean, selectedTab?: Tab, remaining: number }) {
  if (!party) return 'Não existe festa ativa'

  return [
    party.name,
    formatStoredDate(party.date),
    options.showTabBalance && options.selectedTab ? options.remaining > 0 ? `A consumir ${formatCurrency(options.remaining)}` : 'Mínimo consumido' : null,
  ].filter(Boolean).join(' • ')
}

export function findPartyItem<T extends { id: string }>(items: T[] | undefined, id: string | null) {
